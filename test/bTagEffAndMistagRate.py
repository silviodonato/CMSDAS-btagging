...
# b-tagger
bTaggers = ['pfCombinedInclusiveSecondaryVertexV2BJetTags','DeepCSV_btagger']
bTaggersOperatingPointsL = [0.5803,0.1522]
bTaggersOperatingPointsM = [0.8838,0.4941]
bTaggersOperatingPointsT = [0.9693,0.8001]
...
for bi in range(len(bTaggers)):

   # medium operating point
   operatingPoint = bTaggersOperatingPointsM[bi] # CSVv2IVFM
   if options.operating_point == 'loose': operatingPoint = bTaggersOperatingPointsL[bi] # CSVv2IVFL
   if options.operating_point == 'tight': operatingPoint = bTaggersOperatingPointsT[bi] # CSVv2IVFT

   # get 2D b-tag discriminator vs jet pT histograms
   discrVsPt_b_ttbar    = inputFile_ttbar.Get('bTaggingExerciseII/' + bTaggers[bi] + '_b')
   discrVsPt_udsg_ttbar = inputFile_ttbar.Get('bTaggingExerciseII/' + bTaggers[bi] + '_udsg')
   discrVsPt_b_QCD      = inputFile_QCD.Get('bTaggingExerciseII/' + bTaggers[bi] + '_b')
   discrVsPt_udsg_QCD   = inputFile_QCD.Get('bTaggingExerciseII/' + bTaggers[bi] + '_udsg')
   ...
   print 'operatingPoint = '+str(operatingPoint)+', firstbin = '+str(firstbin)+', lastbin = '+str(lastbin)
   ...
   # save the plot
   if options.operating_point == 'loose': c.SaveAs('bTaggingEfficiency_'+bTaggers[bi]+'L.png')
   elif options.operating_point == 'tight': c.SaveAs('bTaggingEfficiency_'+bTaggers[bi]+'T.png')
   else: c.SaveAs('bTaggingEfficiency_'+bTaggers[bi]+'M.png')
   ...
   # save the plot
   if options.operating_point == 'loose': c.SaveAs('MistagRate_'+bTaggers[bi]+'L.png')
   elif options.operating_point == 'tight': c.SaveAs('MistagRate_'+bTaggers[bi]+'T.png')
   else: c.SaveAs('MistagRate_'+bTaggers[bi]+'M.png')
