...
bTaggersTitle = ['CSVv2 b tag discriminator','DeepCSV b tag discriminator']
...
for bi in range(len(bTaggers)):
   ...
   discrVsPt_b    = inputFile.Get('bTaggingExerciseII/' + bTaggers[bi] + '_b')
   discrVsPt_c    = inputFile.Get('bTaggingExerciseII/' + bTaggers[bi] + '_c')
   discrVsPt_udsg = inputFile.Get('bTaggingExerciseII/' + bTaggers[bi] + '_udsg')
   ...
   # light-flavor jets
   discr_udsg.GetXaxis().SetTitle(bTaggersTitle[bi])
   discr_udsg.GetXaxis().SetTitleOffset(1.0)   
   ...
   c.SaveAs(bTaggers[bi]+'.png')
   
   c.SetLogy()
   #c.SaveAs('pfCSVv2IVF_discriminator_log.png')
   c.SaveAs(bTaggers[bi]+'_log.png')
